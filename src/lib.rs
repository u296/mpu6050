    ...
    I: Write<Error = E> + WriteRead<Error = E>, 
{
    ...
    pub fn get_acc_angles(&mut self) -> Result<[f32;2], Mpu6050Error<E>> {
    ...
    fn read_rot(&mut self, reg: u8) -> Result<[f32;3], Mpu6050Error<E>> {
    ...
    pub fn get_acc(&mut self) -> Result<[f32;3], Mpu6050Error<E>> {
        let mut acc = self.read_rot(ACC_REGX_H)?;
        Ok(
            [
                acc[0] / self.acc_sensitivity,
                acc[1] / self.acc_sensitivity,
                acc[2] / self.acc_sensitivity,
            ]
        )
    }

    /// Gyro readings in rad/s
    pub fn get_gyro(&mut self) -> Result<[f32; 3], Mpu6050Error<E>> {
        let mut gyro = self.read_rot(GYRO_REGX_H)?;

        let c = PI_180 / self.gyro_sensitivity;
